...
use aya_obj::generated::{XDP_MMAP_OFFSETS, XDP_STATISTICS};

use super::{IfCtx, IfInfo, SocketFd, SocketMmapOffsets};
use crate::af_xdp::{
    xsk::{XdpMmapOffsets, XdpMmapOffsetsV1, XdpStatistics},
    Errno, LastErrno,
};
...
impl SocketMmapOffsets {
    ...
    pub fn new(sock: &SocketFd) -> Result<Self, Errno> {
        let mut this = SocketMmapOffsets::default();
    ...
    pub fn set_from_fd(&mut self, sock: &SocketFd) -> Result<(), Errno> {
        use crate::af_xdp::xsk::{XdpRingOffsets, XdpRingOffsetsV1};
