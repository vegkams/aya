...
use aya_obj::generated::{
    XDP_RX_RING, XDP_SHARED_UMEM, XDP_TX_RING, XDP_UMEM_COMPLETION_RING, XDP_UMEM_FILL_RING,
    XDP_UMEM_REG,
};

use crate::af_xdp::{
    xsk::{
        ptr_len, BufIdx, DeviceControl, DeviceQueue, DeviceRings, IfCtx, RingCons, RingProd,
        RingRx, RingTx, SockAddrXdp, Socket, SocketConfig, SocketFd, SocketMmapOffsets, Umem,
        UmemChunk, UmemConfig, User, XdpDesc, XdpStatistics, XdpUmemReg,
    },
    Errno, LastErrno,
};
...
impl Umem {
    ...
    pub unsafe fn new(config: UmemConfig, area: NonNull<[u8]>) -> Result<Self, Errno> {
        ...
        assert!(config.inner.frame_size > 0, "Invalid frame size");
    ...
    pub fn frame(&self, idx: BufIdx) -> Option<UmemChunk> {
        let pitch: u32 = self.config.inner.frame_size;
        let idx: u32 = idx.0;
    ...
    pub fn len_frames(&self) -> u32 {
        let area_size = ptr_len(self.umem_buffer.as_ptr()) as u64;
        let count = area_size / u64::from(self.config.inner.frame_size);
    ...
    fn configure(this: &Umem) -> Result<(), Errno> {
        let mut mr = XdpUmemReg {
            ...
            chunk_size: this.config.inner.frame_size,
            headroom: this.config.inner.frame_headroom,
            flags: this.config.inner.flags,
            ..XdpUmemReg::default()
        };
    ...
    pub fn fq_cq(&self, interface: &Socket) -> Result<DeviceQueue, Errno> {
        ...
        let prod = unsafe { RingProd::fill(sock, &map, self.config.inner.fill_size) }?;
        let cons = unsafe { RingCons::comp(sock, &map, self.config.inner.comp_size) }?;
    ...
    pub(crate) fn configure_cq(fd: &SocketFd, config: &UmemConfig) -> Result<(), Errno> {
        if unsafe {
            libc::setsockopt(
                ...
                (&config.inner.comp_size) as *const _ as *const libc::c_void,
                core::mem::size_of_val(&config.inner.comp_size) as libc::socklen_t,
            )
        } != 0
        {
            return Err(LastErrno)?;
        ...
        if unsafe {
            libc::setsockopt(
                ...
                (&config.inner.fill_size) as *const _ as *const libc::c_void,
                core::mem::size_of_val(&config.inner.fill_size) as libc::socklen_t,
            )
        } != 0
        {
            return Err(LastErrno)?;
